...
class UsersController {
  ...
  async login(req, res, next) {
    try {
      ...
      const user = await usersService.get(userId); //on récupère l'user grâce à l'id

      const token = jwt.sign({ user }, config.secretJwtToken, { //on fait passé toutes les informations user dans le token
        expiresIn: "3d",
      });
      res.json({
        token,
